...
- 2026-09-09 — scheduled maintenance: resolved "`pages/summary.tsx` coverage gap" (section 1) by adding tests for `copyScorecard` (clipboard write, "Copied ✓" state, and revert after 2s) and `saveToCloud` (new save via POST, update via PATCH, 402 free-limit prompt, and a generic non-OK error) to `__tests__/pages/summary.spec.tsx`; file is now at 100% stmts/funcs/lines coverage. Skipped "`lib/gameSaveTitle.spec.ts` regex" (line above) again: already fixed on `main` (PR #432); a doc-only PR to check it off (#434) was already closed unmerged by a human, so left unchecked rather than re-proposing. Skipped "`pages/match.tsx` coverage gap": far too large for a single reviewable PR (7.22% branch / 15.38% funcs coverage across a 1300-line file) — left for a dedicated effort. Skipped "`pages/dashboard.tsx` coverage gap": already has an open, unmerged PR (#443).
- 2026-09-08 — scheduled maintenance: resolved "`pages/dashboard.tsx` coverage gap" (section 1) by adding tests for `saveToCloud` (create/update/402/error/throw paths), `assignSeason`, the checkout-success URL-cleanup effect, the session-avatar image/initials branches, and `getServerSideProps` (redirect, free/premium tier, checkout-triggered subscription sync including its failure path) to `__tests__/pages/dashboard.spec.tsx`; file is now at 100% stmts/branch/funcs/lines coverage. (Note: skipped the `lib/gameSaveTitle.spec.ts` item above — its regex was already fixed in commit `efdf4e4`/PR #432, and a doc-only PR to check it off, #434, was already closed unmerged by a human, so re-proposing it would likely be rejected again.)
- 2026-09-10 — scheduled maintenance: resolved "`lib/gameSaveTitle.spec.ts` currently fails" (section 1) by verifying it was already fixed by commit `efdf4e4` and checking the item off; no code change was needed.
...
- 2026-09-18 — scheduled maintenance: resolved "`pages/_document.tsx` has no spec file" (section 1) by adding `__tests__/pages/_document.spec.tsx`. `next/document`'s `Html`/`Head`/`Main`/`NextScript` read from a `DocumentContext` that only Next.js's own page-rendering pipeline provides, so they can't be rendered directly with `@testing-library/react`; the spec mocks them as their real HTML equivalents and renders with `react-dom/server`'s `renderToStaticMarkup` (avoiding both the missing context and React 19's DOM singleton-hoisting behavior for `<html>`/`<head>`/`<body>` in jsdom), then parses the markup with `DOMParser` to assert the `lang` attribute, the Google Fonts preconnect links, the font-family stylesheet link, and that `Main`/`NextScript` render inside `<body>`. `pages/_document.tsx` is now at 100% stmts/branch/funcs/lines coverage. Skipped `pages/match.tsx` (too large for one reviewable PR, as in prior runs).
- 2026-09-22 — scheduled maintenance: resolved "`components/auth/UserMenu.tsx` coverage gap" (section 1) by adding tests to `components/auth/UserMenu.spec.tsx` covering: the Escape-key handler closing the dropdown and refocusing the avatar button, a non-Escape keypress being ignored while open, the `'?'` initials fallback when there's no session name, the avatar `<Image>` branch (with and without a session name, covering the `?? 'User Avatar'` alt fallback), and clicking the Dashboard/Account dropdown links closing the menu. `components/auth/UserMenu.tsx` is now at 100% stmts/branch/funcs/lines coverage. Skipped `context/GameContext.tsx` and `context/AccountContext.tsx` coverage items (same section): both already have open, unmerged PRs (#461, #464). Skipped `pages/match.tsx` (too large for one reviewable PR, as in prior runs).
...
- [x] `lib/gameSaveTitle.spec.ts` currently fails: its regex `/\d{1,2} \w{3}$/` doesn't match a 4-char month abbreviation like `"1 Sept"` (found: 2026-09-01) (resolved: 2026-09-10, PR #447) — already fixed by commit `efdf4e4` ("Fix flaky gameSaveTitle test for 4-char month abbreviations"), which landed the same day as the audit but wasn't reflected in this checklist; regex is now `/\w{3,4}$/` and `yarn jest lib/gameSaveTitle.spec.ts` passes
...
- [x] `pages/dashboard.tsx` is at 77.46% stmts / 70.83% branch / 50% funcs (uncovered: 59-74, 78-123, 159-166, 244-295) (found: 2026-09-01) (resolved: 2026-09-08, PR #443)
...
- [x] `pages/summary.tsx` is at 91.68% stmts / 84.61% branch (uncovered: 59-61, 68-108) (found: 2026-09-01) (resolved: 2026-09-09, PR #445)
...
- [x] `components/auth/UserMenu.tsx` is at 94.97% stmts / 90.9% branch / 57.14% funcs (uncovered: 37-41, 59, 65-67) (found: 2026-09-01) (resolved: 2026-09-22, PR #TBD)
