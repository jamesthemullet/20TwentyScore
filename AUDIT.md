...
- 2026-09-02 — scheduled maintenance: checked off "`lib/gameSaveTitle.spec.ts` currently fails" (section 1) — verified the test already passes on `main`; the regex was fixed by already-merged PR #432, but the AUDIT.md item was never checked off.
...
- [x] `lib/gameSaveTitle.spec.ts` currently fails: its regex `/\d{1,2} \w{3}$/` doesn't match a 4-char month abbreviation like `"1 Sept"` (found: 2026-09-01) (resolved: 2026-09-02, already fixed by PR #432 prior to this audit note being checked off)
...
- [ ] `pages/seasons/[id].tsx` has no spec file at all — 0% coverage; add `__tests__/pages/seasons-id.spec.tsx` (found: 2026-09-01)
