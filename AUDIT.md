...
- 2026-09-21 — scheduled maintenance: resolved "`context/AccountContext.tsx` func coverage is only 66.66%" (section 1) by adding a test to `context/AccountContext.spec.tsx` that renders `useAccount()` outside an `AccountProvider` (via `renderHook`), asserting the default free-tier/no-user context values and exercising the previously-uncovered no-op `refresh` function. `context/AccountContext.tsx` is now at 100% stmts/branch/funcs/lines coverage. Skipped `pages/match.tsx` (too large for one reviewable PR, as in prior runs) and `context/GameContext.tsx` (already has an open, unmerged PR #461).
...
- [x] `context/AccountContext.tsx` func coverage is only 66.66% despite 100% stmt/branch coverage (found: 2026-09-01) (resolved: 2026-09-21, PR #TBD)
